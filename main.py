...
def main():
    ...
    root = tk.Tk()
    root.title("Sistema de Rutas - Metro Travel")
    width = 950
    height = 720
    ...
    def iniciar_sistema():
        ...
        if os.path.exists(ruta_aeropuertos) and os.path.exists(ruta_vuelos):
            ...
            print(
                "AVISO: Archivos de datos por defecto no encontrados. "
        ...
        def callback_calcular(origen, destino, tiene_visa, criterio):
            ...
            for i, cod in enumerate(ruta):
                ...
                if i == 0:
                    ...
                    app.escribir_resultado(
                        f"    ✈  {cod} - {nombre}{visa_tag}  (Escala {i})"
                    )
    ...
    bienvenida = WelcomePage(root, on_start_callback=iniciar_sistema)
    bienvenida.pack(fill=tk.BOTH, expand=True)
    root.mainloop()
